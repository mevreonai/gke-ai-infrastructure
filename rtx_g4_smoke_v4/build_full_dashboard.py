...
# Ensure header tabs are exactly the 4 requested dashboards:
# 1. 9-Panel Infographic
# 2. AllReduce Suite
# 3. AllGather Suite
# 4. ReduceScatter Suite
...
# Replace whatever tabs currently exist in header
import re
...
# Javascript logic: Initializes Tab 1 charts on load, and handles Tab 2, 3, 4 interactive views
script_js = f"""
<script>
  ...
  function renderCollectiveView(coll) {{
    ...
    // 1. Render Dynamic Download Links
    let currentTpKey = (sel === 'compare') ? 'tp16' : sel;
    let tpLabel = (sel === 'tp16') ? 'TP=16' : ((sel === 'tp8') ? 'TP=8' : ((sel === 'tp4') ? 'TP=4' : 'TP'));
    
    let dlHtml = `
      <a href="fresh_benchmark_suite/${{coll}}/${{coll}}_${{currentTpKey}}_sweep.csv" download style="background:rgba(59,130,246,0.2); border:1px solid #3b82f6; color:#93c5fd; font-size:12px; font-weight:600; padding:6px 12px; border-radius:6px; text-decoration:none;">${{tpLabel}} Full Sweep CSV &darr;</a>
      <a href="fresh_benchmark_suite/${{coll}}/${{coll}}_${{currentTpKey}}_175g_native.csv" download style="background:rgba(16,185,129,0.2); border:1px solid #10b981; color:#34d399; font-size:12px; font-weight:600; padding:6px 12px; border-radius:6px; text-decoration:none;">175G Native CSV &darr;</a>
      <a href="fresh_benchmark_suite/${{coll}}/${{coll}}_${{currentTpKey}}_100g.csv" download style="background:rgba(255,255,255,0.06); border:1px solid var(--border); color:#fff; font-size:12px; font-weight:600; padding:6px 12px; border-radius:6px; text-decoration:none;">100G CSV &darr;</a>
      <a href="fresh_benchmark_suite/${{coll}}/${{coll}}_${{currentTpKey}}_50g.csv" download style="background:rgba(255,255,255,0.06); border:1px solid var(--border); color:#fff; font-size:12px; font-weight:600; padding:6px 12px; border-radius:6px; text-decoration:none;">50G CSV &darr;</a>
      <a href="fresh_benchmark_suite/${{coll}}/${{coll}}_${{currentTpKey}}_20g.csv" download style="background:rgba(255,255,255,0.06); border:1px solid var(--border); color:#fff; font-size:12px; font-weight:600; padding:6px 12px; border-radius:6px; text-decoration:none;">20G CSV &darr;</a>
      <a href="fresh_benchmark_suite/${{coll}}/${{coll}}_${{currentTpKey}}_10g.csv" download style="background:rgba(239,68,68,0.2); border:1px solid #ef4444; color:#fca5a5; font-size:12px; font-weight:600; padding:6px 12px; border-radius:6px; text-decoration:none;">10G CSV &darr;</a>
    `;
    if (sel === 'tp8' || sel === 'compare') {{
      dlHtml += `<a href="fresh_benchmark_suite/${{coll}}/${{coll}}_tp8_node_local.csv" download style="background:rgba(139,92,246,0.2); border:1px solid #8b5cf6; color:#c4b5fd; font-size:12px; font-weight:600; padding:6px 12px; border-radius:6px; text-decoration:none;">TP=8 Local PCIe CSV &darr;</a>`;
    }}
    if (sel === 'tp4' || sel === 'compare') {{
      dlHtml += `<a href="fresh_benchmark_suite/${{coll}}/${{coll}}_tp4_socket_local.csv" download style="background:rgba(236,72,153,0.2); border:1px solid #ec4899; color:#fbcfe8; font-size:12px; font-weight:600; padding:6px 12px; border-radius:6px; text-decoration:none;">TP=4 Socket-Local NUMA CSV &darr;</a>`;
    }}
    // Also include standalone comparison CSV and Markdown report
    dlHtml += `
      ...
      <a href="TP16_VS_TP8_LOCAL_${{coll.toUpperCase()}}_REPORT.md" download style="background:rgba(139,92,246,0.3); border:1px solid #8b5cf6; color:#fff; font-size:12px; font-weight:700; padding:6px 12px; border-radius:6px; text-decoration:none;">Executive Report (.md) &darr;</a>
    `;
  ...
  // Initialize Tab 1 (9-Panel Infographic) Charts
  function initTab1Charts() {{
    ...
    const elScaleOut = document.getElementById('chartScaleOut');
    if (elScaleOut) {{
      new Chart(elScaleOut, {{
        type: 'line',
        data: {{
    ...
    const elNetSweep = document.getElementById('chartNetworkSweep');
    if (elNetSweep) {{
      new Chart(elNetSweep, {{
        type: 'bar',
        data: {{
    ...
    const elSendRecv = document.getElementById('chartSendRecv');
    if (elSendRecv) {{
      new Chart(elSendRecv, {{
        type: 'line',
        data: {{
          labels: ['Native', '100G', '50G', '20G', '10G'],
...
print("Successfully restored dashboard to 4 tabs with working Tab 1 charts and collective suites!")
