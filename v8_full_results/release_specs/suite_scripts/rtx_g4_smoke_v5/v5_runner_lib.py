...
def cli_help(vllm, parts):
    res = run_capture([vllm] + list(parts) + ["--help=all"], timeout=120)["out"]
    if not res or (len(parts) > 0 and parts[0] == "serve" and "--tensor-parallel-size" not in res):
        fallback = run_capture([vllm] + list(parts) + ["--help"], timeout=120)["out"]
        if fallback: res = (res or "") + "\n" + fallback
    return res or ""
