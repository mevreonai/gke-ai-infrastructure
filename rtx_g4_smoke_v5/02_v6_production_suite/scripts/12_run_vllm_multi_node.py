...
def ssh_base(key, host): return ["ssh","-i",key,"-o","StrictHostKeyChecking=no","-o","UserKnownHostsFile=/dev/null",host]
def scp_base(key): return ["scp","-i",key,"-o","StrictHostKeyChecking=no","-o","UserKnownHostsFile=/dev/null"]
...
def main():
    ...
    for ci,case in enumerate(cases):
        ...
        rec={"name":case["name"],"groups":case.get("groups",[]),"purpose":case.get("purpose"),"tp":case["tp"],"pp":case.get("pp",1),
             ...
             "kv_cache_dtype":resolved(case,cfg,"kv_cache_dtype","auto"),"server_command":server_cmd,"benchmarks":[]}
        if args.dry_run:
            top["cases"].append(rec); continue
        server_log=(cdir/"server.log").open("w",buffering=1); server_proc=None; local_sampler=None; remote_pid=None
        try:
            ...
            for bi,b in enumerate(case["benchmarks"]):
                ...
                if warm.get("ran") and warm.get("exit_code")!=0:
                    rec["benchmarks"].append({**b,"status":"WARMUP_FAILED","exit_code":warm.get("exit_code")}); continue
                ...
                rec["benchmarks"].append({**b,"status":"COMPLETED" if rc==0 else "FAILED","command":bench_cmd,"start":t0,"end":t1,"exit_code":rc,
                                          "result_json":str(bdir/result_file),"remote_sampler_error":remote_err,"warmup_separate":True,**qm,**lens})
        except Exception as e: rec["error"]=repr(e)
        finally:
            if local_sampler: stop_proc(local_sampler)
            if remote_pid: stop_remote_sampler(args.ssh_key,args.node1_ip,remote_pid,"/tmp/does_not_matter",cdir/"remote_sampler_unrecovered.jsonl")
